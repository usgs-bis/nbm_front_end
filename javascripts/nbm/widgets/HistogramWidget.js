...
function HistogramWidget(config, bap) {
    ...
    this.buildChart = function (chartData, id) {
        ...
        function toolTipLabel(d, buk) {
            ...
            let count = `Count: <label>${parseInt(d.count)} </label> of <label>${parseInt(totalCount)} </label>( ~ ${percentage}%)<br />  *count is values that occur ${dateFromDay(2018, d.day * buk - buk)} to ${dateFromDay(2018, d.day * buk - 1)} <br /> for all selected years(${startYear} to ${endYear}). <br />`
            if (buk == 1) {
