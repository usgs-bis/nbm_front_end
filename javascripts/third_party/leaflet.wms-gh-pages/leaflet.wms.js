...
wms.Overlay = L.Layer.extend({
    ...
    'update': function() {
        ...
        if(!pane) {
            pane = 'overlayPane';
            console.log('There was no pane option present, this null check is valid. Remove this console.log from leaflet.wms.js line 347.');
        }
        var overlay = L.imageOverlay(url, bounds, {'opacity': 0, 'pane': pane});
        ...
        function _swap() {
            ...
            overlay.setOpacity(
                this.options.opacity ? this.options.opacity : .5
            );

            //since there is no zIndex for overlays in leaflet if this overlay has a config setting
            // of bringToBack: true we use bringToBack() to move it to the bottom of all our overlays
            // (will likely need to be changed if we bring in more layers like this)
